...
it('returns empty achievements and Bronze badge for a user with no purchases', function (): void {
    ...
    $this->getJson("/api/users/{$user->id}/achievements")
        ...
        ->assertJson([
            ...
            'next_available_achievements'    => ['First Steps'],
...
it('unlocks First Steps after 1 purchase', function (): void {
    ...
    $achBefore          = app(BadgeService::class)->currentBadge($user);
    $achievementService->evaluate($user);
    app(BadgeService::class)->evaluate($user, $achBefore);
...
// ──────────────────────────────────────────────────────────────────────────────
// 3. After 5 purchases – "Regular Shopper" is unlocked
// ──────────────────────────────────────────────────────────────────────────────

it('unlocks Regular Shopper after 5 purchases', function (): void {
    $user = User::factory()->create();
    ...
    $this->getJson("/api/users/{$user->id}/achievements")
        ->assertOk()
        ->assertJsonFragment(['unlocked_achievements' => ['First Steps', 'Regular Shopper']])
        ->assertJsonFragment(['current_badge' => 'Silver']);
});

// ──────────────────────────────────────────────────────────────────────────────
// 4. After 10 purchases – "Loyal Customer" is unlocked; badge is Silver (3 ach.)
// ──────────────────────────────────────────────────────────────────────────────
...
it('unlocks Loyal Customer after 10 purchases and badge is Silver', function (): void {
    ...
    $this->getJson("/api/users/{$user->id}/achievements")
        ->assertOk()
        ->assertJsonFragment(['current_badge' => 'Silver'])
        ->assertJsonPath('unlocked_achievements.2', 'Loyal Customer');
});
...
// ──────────────────────────────────────────────────────────────────────────────
// 8. Duplicate purchases do not re-unlock an already-unlocked achievement
// ──────────────────────────────────────────────────────────────────────────────
...
it('does not re-unlock an already-unlocked achievement on duplicate evaluate calls', function (): void {
    ...
    $achievementService = app(AchievementService::class);
    $achievementService->evaluate($user);
    $achievementService->evaluate($user); // call again – must be idempotent
...
// ──────────────────────────────────────────────────────────────────────────────
// 9. API returns 404 for a non-existent user
// ──────────────────────────────────────────────────────────────────────────────
